...
class SaveDomainCase(testing.AsyncTestCase):
    ...
    def setUp(self):
        ...
        db.RedisClient.setup(known_domains=DOMAINS_SET, io_loop=self.io_loop)
    ...
    def test_save_domain(self):
        res = yield gen.Task(db.RedisClient().save_domain, 'tornadoweb.org')
...
class CheckDomainCase(testing.AsyncTestCase):
    ...
    def test_is_known_domain(self):
        res = yield gen.Task(db.RedisClient().is_known_domain, 'tornadoweb.org')
        self.assertTrue(res)

    @testing.gen_test
    def test_remove_domain(self):
        res = yield gen.Task(db.RedisClient().remove_domain, 'tornadoweb.org')
...
class PutTaskCase(testing.AsyncTestCase):
    ...
    def setUp(self):
        ...
        db.RedisClient.setup(tasks_queue=TASKS_LIST, io_loop=self.io_loop)
    ...
    @testing.gen_test
    def test_put(self):
        yield gen.Task(db.RedisClient().put_task, 'http://tornadoweb.org/')
...
class GetTaskCase(testing.AsyncTestCase):
    ...
    @testing.gen_test
    def test_is_known_domain(self):
        res = yield gen.Task(db.RedisClient().get_task)
        self.assertEqual('http://tornadoweb.org/', res)


    @testing.gen_test
    def test_count(self):
        res = yield gen.Task(db.RedisClient().tasks_count)
        self.assertEqual(1, res)
...
class SaveReportCase(testing.AsyncTestCase):
    ...
    def setUp(self):
        ...
        db.RedisClient.setup(reports_hash=REPORTS_HASH, io_loop=self.io_loop)
    ...
    def test_save_report(self):
        res = yield gen.Task(db.RedisClient().save_report, 'http://tornadoweb.org/', 'bar')
        self.assertEqual(1, res)
...
class LoadReportCase(testing.AsyncTestCase):
    ...
    def test_load_report(self):
        res = yield gen.Task(db.RedisClient().load_report, 'http://tornadoweb.org/')
        self.assertEqual('foo', res)

    @testing.gen_test
    def test_reports_count(self):
        res = yield gen.Task(db.RedisClient().reports_count)
        self.assertEqual(1, res)
